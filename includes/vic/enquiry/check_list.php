...
if(isset($_POST['generate_load_list']))
{
    ...
    $sql = "DELETE FROM ver_chronoforms_data_letters_vic WHERE clientid='{$clientid}' AND template_type='check list - gutter flashing' ";   
    mysql_query($sql);  
     
    $sql = "DELETE FROM ver_chronoforms_data_letters_vic WHERE clientid='{$clientid}' AND template_type='check list' ";   
    mysql_query($sql); 
...
<!-- &nbsp;&nbsp;<a id="frame_create_po" class="btn btn-s" rel="nofollow" title="PDF" onclick="window.open(this.href,'win2','status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=auto,height=auto,directories=no,location=no'); return false;" href="index.php?projectid=<?php echo $projectid; ?>&clientid=<?php echo $ClientID; ?>&titleID=<?php echo $load_list_titleID; ?>&option=com_chronoforms&tmpl=component&chronoform=Load_List" style="margin-right:5px; " > Create Load List </a>  -->

  <input type="button" name="generate_load_list" value="Create Load List " class="btn" style="margin: 14px 0px 10px 25px;" onclick="$('#form_generate_checklist').submit();"/>


<ul id="tbl-imgpic" class="picture-block" style="margin-top: 15px;">
<?php
...
<tr  >
  <td colspan="2" width="50%"  >
    <b>Client Name:</b>&nbsp;'; 
    if($contract["is_builder"]==1){ $head .= $contract["builder_name"]; }else{ $head .= $contract["client_firstname"]." ".$contract["client_lastname"];}

  $head .='   
  </td> 
  <td colspan="2" width="50%"   >
    <b>Installer:</b>'. $contract["erectors_name"].'
  </td  >
</tr>
...
<tr>
  <td colspan="2" style="  vertical-align:top;" rowspan="2"  >
    <span>Site:</span>'. $contract["site_address1"].'<br/>'.$contract["site_address2"].'
  </td>  
...
<tr  >
  <td colspan="2" width="50%"  >
    <b>Client Name:</b>&nbsp;'; 
    if($contract["is_builder"]==1){ $head_2 .= $contract["builder_name"]; }else{ $head_2 .= $contract["client_firstname"]." ".$contract["client_lastname"];}
...
<table class=""   cellspacing="0" style="border-collapse:collapse;font-size:9pt; margin-top:5px;" >
  <tr >
      <th width="290"   border="1" style="text-align: center;" >
         <b>Description</b> 
      </th>
      <th width="40" border="1" style="text-align: right;">
         <b>Qty</b> 
      </th>
      <th width="60" border="1" style="text-align: right;">
         <b>Length</b> 
      </th>
...
<table class=""   cellspacing="0" style="border-collapse:collapse;font-size:9pt;" >
  <tr >
      <th width="285"   border="1" style="text-align: center;" >
         <b>Description</b> 
      </th>
      <th width="40" border="1" style="text-align: right;">
         <b>Qty</b> 
