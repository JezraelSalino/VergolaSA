...
function pagination($current_page_number, $total_records_found, $query_string = null)
{
	...
	for ($total_pages = ($total_records_found/NUMBER_PER_PAGE); $total_pages > 0; $total_pages--)
	{
		if ($page != $current_page_number)
			echo "<a href=\"" . "migrated-external/inventory-listing-ext" . "?page=$page" . (($query_string) ? "&$query_string" : "") . "\">";
...
<form action='" . JRoute::_($url) . "' method='post' style='float:none; width:90%;'>
	...
	<input type='button' class='add-btn' onclick=location.href='" . JURI::base() . "migrated-external/inventory-listing-ext/inventory-updatelist-ext' value='Add New'>
</form>
...
while ($record = mysql_fetch_assoc($loop)){
    echo "<tr class='pointer' onclick=location.href='" . JURI::base() . "migrated-external/inventory-listing-ext/inventory-updatelist-ext?inventoryid={$record['inventoryid']}' >";
