...
if($contract_status=="drawing_approval"){ 
    $contract_status_filter = " AND cv.drawing_approve_date IS NULL AND cv.check_measure_date IS NOT NULL AND cv.handover_date IS NULL ";
}else if($contract_status=="development_approval"){ 
    $contract_status_filter = " AND cs.dev_approval_date IS NULL AND cv.drawing_approve_date IS NOT NULL AND cv.handover_date IS NULL "; //drawing_prepare_date
...
if ($contract_status == "drawing_approval"){
    $contract_status_filter = " AND cv.drawing_approve_date IS NOT NULL 
                                AND cs.planning_application_date IS NULL
                                AND cs.bldg_rules_application IS NULL
                                AND cs.dev_approval_date IS NULL
    ...
    $contract_status_filter = " AND cs.dev_approval_date IS NOT NULL AND cv.drawing_prepare_date IS NOT NULL AND cv.handover_date IS NULL   
                                AND cv.fw_orderdate IS NULL
                                AND cv.louvers_ordered IS NULL
                                AND cv.fw_complete IS NULL
                                AND cv.louvers_complete IS NULL
                                ...
                                AND cv.job_end_date IS NULL";
    if ($searchdate) {
        $date_filter = " AND DATE(cs.dev_approval_date) BETWEEN DATE('{$frdate}') AND DATE('{$todate}')   
                            AND cv.fw_orderdate IS NULL
                            AND cv.louvers_ordered IS NULL
                            AND cv.fw_complete IS NULL
                            AND cv.louvers_complete IS NULL
...
//     }
...
}else if ($contract_status == "planning_application") {
    $contract_status_filter = " AND cs.planning_application_date IS NOT NULL AND cv.handover_date IS NULL 
                                AND cs.bldg_rules_application IS NULL
                                AND cs.dev_approval_date IS NULL
                                AND cv.fw_orderdate IS NULL
                                AND cv.louvers_ordered IS NULL
                                ...
                                AND cv.job_end_date IS NULL";
    if ($searchdate) {
        $date_filter = " AND DATE(cs.planning_application_date) BETWEEN DATE('{$frdate}') AND DATE('{$todate}') 
                            AND cs.bldg_rules_application IS NULL
                            AND cs.dev_approval_date IS NULL
                            AND cv.fw_orderdate IS NULL
                            AND cv.louvers_ordered IS NULL
...
}else if ($contract_status == "bldg_rules_application") {
    $contract_status_filter = " AND cs.bldg_rules_application IS NOT NULL AND cv.handover_date IS NULL 
                                AND cs.dev_approval_date IS NULL
                                AND cv.fw_orderdate IS NULL
                                AND cv.louvers_ordered IS NULL
                                AND cv.fw_complete IS NULL
                                ...
                                AND cv.job_end_date IS NULL";
    if ($searchdate) {
        $date_filter = " AND DATE(cs.bldg_rules_application) BETWEEN DATE('{$frdate}') AND DATE('{$todate}') 
                            AND cs.dev_approval_date IS NULL    
                            AND cv.fw_orderdate IS NULL
                            AND cv.louvers_ordered IS NULL
                            AND cv.fw_complete IS NULL
    ...
    $contract_status_filter = " AND cv.fw_complete IS NOT NULL AND cs.dev_approval_date IS NOT NULL AND cv.handover_date IS NULL 
                                AND cv.install_date IS NULL
                                AND cv.job_end_date IS NULL";
    if ($searchdate) {
...
}else if ($contract_status == "handover_date") {
    $contract_status_filter = " AND cv.handover_date IS NOT NULL ";
    if ($searchdate) {
        $date_filter = " AND DATE(cv.handover_date) BETWEEN DATE('{$frdate}') AND DATE('{$todate}') ";
    }
}else if ($contract_status == "warranty_end_date") {
    $contract_status_filter = " AND cv.warranty_end_date IS NOT NULL ";
    if ($searchdate) {
        $date_filter = " AND DATE(cv.warranty_end_date) BETWEEN DATE('{$frdate}') AND DATE('{$todate}') ";
    }
// end Insert new condition

}else if ($contract_status == "framework_ordered") {
    $contract_status_filter = " AND cv.fw_orderdate IS NOT NULL AND cs.dev_approval_date IS NOT NULL AND cv.handover_date IS NULL 
                                AND cv.fw_complete IS NULL
                                AND cv.install_date IS NULL
                                AND cv.job_end_date IS NULL"; /*was using fw_complete as getting resut, changed to framework_ordered as per suggested by Les Lim, */
    if ($searchdate) {
    ...
    $contract_status_filter = " AND cv.install_date IS NOT NULL AND cv.handover_date IS NULL 
                                AND cv.job_end_date IS NULL"; /*query as per suggested by Les Lim*/
    if ($searchdate)
        $date_filter = " AND DATE(cv.install_date) BETWEEN DATE('{$frdate}') AND DATE('{$todate}') 
                            AND cv.job_end_date IS NULL";
    }
...
echo " 
        ...
        <label class='input' style=''>  
            <select name='contract_status' style='width: 160px;'> 
                ...
                <option value='planning_application' ". ($contract_status=='planning_application' ? 'selected':'').">Planning Application</option>
                <option value='bldg_rules_application' ". ($contract_status=='bldg_rules_application' ? 'selected':'').">Building Rules Application</option>
                ...
                <option value='handover_date' ". ($contract_status=='handover_date' ? 'selected':'').">Handover</option>
                <option value='warranty_end_date' ". ($contract_status=='warranty_end_date' ? 'selected':'').">Warranty End</option>
            </select>
        </label> 
...
echo " <label class='input'><span>CR</span><input type='input' name='search_ID' value='".($search_ID!=''?$search_ID:"")."' style='border:1px solid #7C7D7F;' ></label>";
...
if(HOST_SERVER=="Victoria" || HOST_SERVER=="LA"){
    ...
    if(isset($_POST['download_pdf'])==false){   
        ...
        $html .= "<table border=\"1\" cellpadding=\"1\" ><tbody><tr >".($is_admin?"<th width=\"50\">Consultant</th>  ":"")."<th width=\"50\">Contract ID</th><th width=\"50\">CRV</th><th width=\"80\">Client Name</th><th width=\"100\">Site Address</th> <th width=\"65\">Contract Date</th><th width=\"60\">Total Price</th> <th width=\"50\">Check Measure Date</th> <th width=\"50\">Check Measurer</th> <th width=\"50\">Drawing Approval</th><th width=\"50\">Permit Application</th>";
    ...
    while ($record = mysql_fetch_assoc($loop)) {
        ...
        ($record['is_builder']==1?
        "<td>".(isset($_POST['download_pdf'])?addslashes($record['builder_name']):$record['builder_name'])."</td>":"<td>".(isset($_POST['download_pdf'])?addslashes($record['client_name']):$record['client_name'])."</td>").
    ...
    if(isset($_POST['download_pdf'])==true){    
     ...
     while ($record = mysql_fetch_assoc($loop)) {
        ...
        ($is_admin==1?"<td>".(isset($_POST['download_pdf'])?addslashes($record['sales_rep']):$record['sales_rep'])."</td>":"").
        "<td>{$record['projectid']}</td><td>{$record['clientid']}</td>".
        /*($record['is_builder']==1?"<td>".(isset($_POST['download_pdf'])?addslashes($record['builder_name']):$record['builder_name'])."
            ...
            </td>":"<td>".(isset($_POST['download_pdf'])?addslashes($record['client_name']):$record['client_name'])."</td>").
        "<td>".(isset($_POST['download_pdf'])?addslashes($record['site_streetno'].' '.$record['site_streetname'].' '.$record['site_address']):$record['site_streetno'].' '.$record['site_streetname'].' '.$record['site_address'])."</td>" . */
        ($record['is_builder']==1?
            "<td >
