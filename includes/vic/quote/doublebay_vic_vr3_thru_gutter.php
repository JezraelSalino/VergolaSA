...
function generateRowItemVR3G($section, $fw=null){ //type = double bay vr2, double bay vr3, double bay vr2 - gutter
	...
	if($section=="frame"){
		...
		//error_log(print_r($framework,true), 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
		while ($r = mysql_fetch_assoc($framework)) {
			...
			$tr .= "<tr>"; 
					...
					if(  (strtolower($item["category"]) == "beams" || strtolower($item["category"])=="posts" || strtolower($item["category"])=="beam fixings" || strtolower($item["category"])=="intermediate")  ){ //
						...
						//error_log("2st TR: ".$tr, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
					}else{
		...
		while ($r = mysql_fetch_assoc($framework)) {
			...
			$item = mysql_fetch_assoc(mysql_query("SELECT * FROM  {$inventory_table} AS i  WHERE inventoryid='{$r["inventoryid"]}' " )); 
			//error_log(" Item: ".$item['inventoryid'] ."-".$r['qty'], 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log'); 
			$tr .= "<tr>"; 
			...
			$tr .= "<tr>"; 
					...
					if($isSecond == 1 || $isThird == 1 || $isFourth == 1){ //($item["inventoryid"]=="IRV43" || $item["inventoryid"]=="IRV46" )  && 
						...
						if($isFirst_IRV44){
					 	...
					 	$isFirst_IRV44 = 0;
					}else if( $item["inventoryid"]=="IRV280"){ //error_log("2nd", 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
		...
		while ($r = mysql_fetch_assoc($framework)) { 
			...
			$tr .= "<tr class=\"tr-vergola\">"; 
				...
				if($item["inventoryid"] == "IRV61"){
					...
					$tr .= "<td class=\"td-qty\"><input type=\"text\" {$itemQty} class=\"qtylen ".(($isFirst||$isSecond)?"louver-item-qty":"")."\"   name=\"qty[]\" value=\"{$r['qty']}\"></td>";  
				}
	...
	//error_log("Return: ".$tr, 3,'C:\\xampp\htdocs\\vergola_contract_system_v4_sa\\my-error.log');
	return $tr;
